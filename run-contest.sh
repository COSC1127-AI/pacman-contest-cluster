...
NOW=`date +"%Y-%m-%d--%H-%M"`
LOG_FILE="contest-feedback-${NOW}.log"
...
# # Secondary repos
# REPO_FILE2=ai20-contest-repos2.csv
# TAG2=submission2
# TIMESTAMP_FILE2=ai20-contest-timestamps2.csv
...
main_function() {
  # First, get into the dir where this folder is!
  cd $DIR_SCRIPT
  ...
  # Clone the secondary repos using the seceondary tag (e.g., "test-submission2")
  # OPTIONS=( --file-timestamps $TIMESTAMP_FILE2 $REPO_FILE2 $TAG2 $SUBMISSIONS )
  # CMD=(python $DIR_CLONER/git_clone_submissions.py ${OPTIONS[@]})
  # echo "=================================================="
  # echo ${CMD[@]} 
  # echo "=================================================="
  ...
  echo ${CMD[@]} 
  echo "=================================================="
  # python3 $DIR_CLUSTER/pacman_contest_cluster.py "${OPTIONS[@]}" 
  "${CMD[@]}"
}
